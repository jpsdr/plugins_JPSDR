...
static uint8_t CreateMTData(MT_Data_Info_AutoYUY2 MT_Data[],int output,uint8_t threads_number,uint8_t max_threads,int32_t size_x,int32_t size_y)
{
	...
	int32_t tab_dh_UV[MAX_MT_THREADS];
	int32_t current_dh;
	uint8_t current_i;
	...
	MT_Data[0].src_UV_h_min=0;
	MT_Data[0].src_UV_h_max=tab_dh_UV[0];
	MT_Data[0].dst_UV_h_min=0;
	MT_Data[0].dst_UV_h_max=tab_dh_Y[0];
	...
	for (uint8_t i=1; i<max; i++)
	{
		...
		MT_Data[i].src_UV_h_max=MT_Data[i].src_UV_h_min+tab_dh_UV[i];
		MT_Data[i].dst_UV_h_min=MT_Data[i-1].dst_UV_h_max;
		MT_Data[i].dst_UV_h_max=MT_Data[i].dst_UV_h_min+tab_dh_Y[i];	
	}
