...
static void resize_v_avx2_planar_uint8_pix16(BYTE* __restrict dst8, const BYTE* src8, int dst_pitch, int src_pitch, ResamplingProgram* program, int width, int bits_per_pixel, int MinY, int MaxY, const int* pitch_table, const uint8_t range, const bool mode_YUY2)
{
	...
	for (int y = MinY; y < MaxY; y++)
	{
		...
		for (int x = 0; x < width; x += 16)
		{
			...
			_mm_stream_si128(reinterpret_cast<__m128i*>(dst8 + x), result128);
		}
		dst8 += dst_pitch;
...
static void resize_v_avx2_planar_uint8_pix32(BYTE* __restrict dst8, const BYTE* src8, int dst_pitch, int src_pitch, ResamplingProgram* program, int width, int bits_per_pixel, int MinY, int MaxY, const int* pitch_table, const uint8_t range, const bool mode_YUY2)
{
    ...
    for (int y = MinY; y < MaxY; y++)
	{
        ...
        for (int x = 0; x < width; x += 32)
		{
            ...
            _mm_stream_si128(reinterpret_cast<__m128i*>(dst8 + x), result128);
            _mm_stream_si128(reinterpret_cast<__m128i*>(dst8 + x + 16), result128_2);
...
void resize_h_planar_float_avx2_permutex_vstripe_ks4_pix16(BYTE* dst8, const BYTE* src8, int dst_pitch, int src_pitch, ResamplingProgram* program, int width, int height, int bits_per_pixel, const uint8_t range, const bool mode_YUY2)
{
  ...
  for (int y_from = 0; y_from < height; y_from += max_scanlines) {
    int y_to = min((int)(y_from + max_scanlines), height);
    // Reset current_coeff for the start of the stripe
    const float* __restrict current_coeff = program->pixel_coefficient_float; // +iYstart * filter_size;
